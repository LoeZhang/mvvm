...
/**
 * 简易ListActivity（支持下拉刷新&上拉加载）
 *
 * @author 章路顺
 * @since 2020/7/3-20:00
 */
abstract class SimpleListActivity<BEAN> : SimpleActivity()
{
    val listController = object: ListController<BEAN>()
    {
        override fun loadData(isRefresh: Boolean) = this@SimpleListActivity.loadData(isRefresh)

        override fun getListAdapter() = this@SimpleListActivity.getListAdapter()
    }
